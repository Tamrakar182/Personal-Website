...
class InfiniteGridMenu {
  ...
  private render(): void {
    ...
    if (this.isDarkMode) {
      // Dark theme background (hsl(210 11.1% 3.53%))
      gl.clearColor(0.035, 0.035, 0.039, 1.0);
    } else {
    ...
    // Make sure we're using the correct item count and atlas size
    const itemCount = Math.max(1, this.items.length);

    gl.uniform1i(this.discLocations.uItemCount, itemCount);
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  const [sketch, setSketch] = useState<InfiniteGridMenu | null>(null);
